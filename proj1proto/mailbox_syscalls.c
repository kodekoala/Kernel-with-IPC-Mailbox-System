  ...
  list_for_each(currBox, &mailBoxes) { //loop mailboxes
    ...
    if (box -> boxId == id) {
      ...
      if (box -> encryption == 0) {
        ...
        memcpy (&kernelMsg[0], &msg[0], sizeof(msg));
...
long xorDecrypt(unsigned char * boxMsg, unsigned char *kernelMsg, unsigned char * msg, long n, uint32_t * key){
  ...
  //In kernel code we need to copy to kernel memory
  kernelMsg = (unsigned char *) malloc (newLen*sizeof(unsigned char));
  memcpy( &kernelMsg[0], &boxMsg[0], sizeof(boxMsg));
...
int main(void) {
  ...
  unsigned char theMsg[] = {0xDE, 0xAD, 0xBE, 0xEF, 0x12, 0x34};
  ...
  unsigned char * usrmsg = (unsigned char * ) malloc(6 * sizeof(unsigned char));

  send_msg_421(50, msg, 6, key);
  printf("=============================\n");
